...
- **読み込み**: コードエリア左下のボタン（旧「拡張機能を追加」）を押すと、そのまま zip の選択画面が開きます。
  選んだ zip は実行される前に検査され、インストール確認画面（セキュリティ検査の結果）が表示されます。
- **管理**: メニューの「編集」→「プラグイン…」で、インストール済みプラグインの有効／無効の切り替えと削除ができます。
