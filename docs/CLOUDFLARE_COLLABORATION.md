...
ブロック操作は参加者ごとに異なる内部ターゲットIDへ依存せず、ステージ種別、プロジェクト内の順序、名前から受信側のスプライトを解決します。画像、音声、動画、モデル、フォントの構成が変わった場合は、アセットを含む最新プロジェクトコピーを保存してオンライン参加者へチャンク配信し、コピー作成後に届いたブロック操作を続けて再適用します。これにより、共同編集開始後に追加した画像なども他の参加者と、後から参加するメンバーの両方へ引き継がれます。
