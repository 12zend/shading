...
ブロック操作は参加者ごとに異なる内部ターゲットIDへ依存せず、ステージ種別、プロジェクト内の順序、名前から受信側のスプライトを解決します。接続変更に加えてワークスペース上の座標移動も共有します。同じブロック群を別の参加者が操作している間は短いリースでロックし、競合した変更はサーバーで拒否して送信元でも自動的に戻します。すべてのルーム更新は直列化し、切断中のブロック変更もローカルだけに残さず取り消します。招待参加者では、招待リンクから取得したプロジェクトの読み込みが完了するまでリアルタイム操作を待機させ、その基準プロジェクトへ順番に適用します。ワークスペースの準備前に届いた操作も破棄せず、準備後に再適用します。画像、音声、動画、モデル、フォントの構成が変わった場合は、アセットを含む最新プロジェクトコピーを保存してオンライン参加者へチャンク配信し、コピー作成後に届いたブロック操作を続けて再適用します。これにより、共同編集開始後に追加した画像なども他の参加者と、後から参加するメンバーの両方へ引き継がれます。

プロジェクトの最大時間、レンダリングフレームレート、レンダリング解像度も共同編集設定です。サーバーが設定の版番号を管理し、変更を全参加者へ即時配信します。同時変更で古い版が届いた場合は受け付けず、最新設定へ戻すため、通信順序による上書きを防ぎます。初回プロジェクト読み込み中に設定を受信した場合は、プロジェクトを読み込んだ後に適用します。
...
チームIDのパス自体はパスワードではありません。以後の権限は、招待トークンとブラウザーごとの参加トークンで検証します。招待トークンはURLフラグメントに置かれるため、HTTPリクエスト、アクセスログ、Refererには送信されません。サーバーにはSHA-256ハッシュだけを保存し、有効期限は7日です。その期間内は同じ招待URLを複数人・複数回利用でき、期限切れ時に削除します。また、新しく招待された参加者には、保存済みの古いコピーではなく、オンライン中のいずれかの管理者から取得した最新プロジェクトだけを初回同期します。同期が終わるまではブロック編集を拒否します。
