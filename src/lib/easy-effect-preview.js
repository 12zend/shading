...
// Renders Easy effect thumbnails with the real PenFX blocks on a private WebGL 2 context. The preset's steps are
// queued through the live PenFX instance while its effect capture is open, so every step keeps the program
// overrides and argument handling of its standalone block, and the captured effects then run against a sandbox
// engine whose only surface is the preview image. The stage renderer's context and pen layer are never touched.

const PREVIEW_BUFFER_ID = 'easy-preview';
const PREVIEW_CONTEXT = Object.freeze({resources: Object.freeze({depth: null}), targetId: null});

class EasyEffectPreviewRenderer {
    constructor (penFX) {
        this.penFX = penFX;
    ...
    // Applies `opcode`'s preset to the snapshot and draws it into the target canvas. `null` draws the input.
    // Returns false when a step failed, in which case the thumbnail shows whatever the other steps produced.
    draw (opcode, preset, targetCanvas) {
        ...
        try {
            ...
            if (preset) {
                ...
                try {
                    penFX.runEasyPreset(opcode, preset.id, 1, null);
                } finally {
...
const getEasyEffectPreviewRenderer = penFX => {
    if (!renderers.has(penFX)) renderers.set(penFX, new EasyEffectPreviewRenderer(penFX));
    return renderers.get(penFX);
};

export {EasyEffectPreviewRenderer, getEasyEffectPreviewRenderer};
