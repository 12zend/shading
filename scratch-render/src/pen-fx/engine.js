    ...
    class PenFXEngine {
        ...
        noteDrawBounds (skin, x, y, width, height) {
            ...
            const next = [Math.max(0, Math.floor(x)), Math.max(0, Math.floor(y)),
                Math.min(this.width, Math.ceil(x + width)), Math.min(this.height, Math.ceil(y + height))];
            if (!entry.bounds.length) entry.bounds = next;
            else entry.bounds = [Math.min(entry.bounds[0], next[0]), Math.min(entry.bounds[1], next[1]),
        ...
        beginGroup () {
            ...
            this.groupStack.push({
                bounds: this.disablePenBounds ? null : [],
                buffer: staging,
        ...
        endGroup (options = {}) {
            ...
            const parentBounds = parentGroup && parentGroup.bounds;
            if (stillStaged) {
                ...
                if (shouldComposite && (entry.expandedOutput || !entry.bounds || entry.bounds.length) && this._prepare(false, false) === skin) {
                    if (entry.expandedOutput) {
                        ...
                        (!this.disableDirectGroupBlend && typeof gl.blendFuncSeparate === 'function' &&
                            [0, 1, 3].includes(blendIndex))) {
                        ...
                        gl.enable(gl.BLEND);
                        gl.blendEquation(gl.FUNC_ADD);
                        if (blendIndex === 1) gl.blendFuncSeparate(gl.ONE, gl.ONE, gl.ONE, gl.ONE_MINUS_SRC_ALPHA);
                        else if (blendIndex === 3) {
                            gl.blendFuncSeparate(gl.ONE, gl.ONE_MINUS_SRC_COLOR, gl.ONE, gl.ONE_MINUS_SRC_ALPHA);
                        } else gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA);
                        const target = skin._framebuffer.framebuffer || skin._framebuffer;
                        this.withProgramOverrides(null, () => {
                            if (opacity === 1) this._render(this._program('copy'), target,
                                [{name: 'u_image', texture: entry.texture}], {}, []);
                            else {
                                if (!this.groupOpacityProgram) this.groupOpacityProgram = this._createProgram(`
                                    precision highp float;
                                    varying vec2 v_uv;
                                    uniform sampler2D u_image;
                                    uniform float u_opacity;
                                    void main() { gl_FragColor = texture2D(u_image, v_uv) * u_opacity; }
                                `);
                                this._render(this.groupOpacityProgram, target,
                                    [{name: 'u_image', texture: entry.texture}], {u_opacity: opacity}, []);
                            }
                        });
                        this._markSkinChanged(skin);
                    } else {
        ...
        _render (program, framebuffer, samplers, uniforms, integerUniforms) {
            ...
            for (const name in uniforms) {
                ...
                const location = this._location(program, name === 'u_lensSamples' ? 'u_lensSamples[0]' : name);
                if (integerUniforms.indexOf(name) !== -1) {
                    ...
                    if (name === 'u_lensSamples') gl.uniform3fv(location, value);
        ...
        // Sample the pen attachment directly, then exchange ownership with a work attachment.
        // This removes the full-resolution input copy without a VM yield or a CPU readback.
        _directSinglePass (program, uniforms, integerUniforms, blendMode) {
            const copyName = this.programOverrides && this.programOverrides.copy || 'copy';
            if (!this._canRenderDirectly(blendMode) || this.groupEffectScope === 'expanded' ||
                String(this.programSources[copyName]).trim() !== this.baseProgramSources.copy.trim()) return false;
            const skin = this._prepare(false);
            if (!skin) return true;
            const source = skin._texture;
            const output = this.textures[0];
            const group = this.groupStack.find(entry => entry.texture === source);
            const matte = this.matteStack.find(entry => entry.source.texture === source ||
                entry.mask && entry.mask.texture === source);
            ...
            if (group && group.bounds) {
                ...
                if (padding !== null) bounds = [Math.max(0, group.bounds[0] - padding),
                    ...
                    Math.min(this.height, group.bounds[3] + padding)];
            }
            ...
            try {
                ...
                if (bounds) {
                    this._clearTransparent(this.framebuffers[0]);
                    gl.enable(gl.SCISSOR_TEST);
                    gl.scissor(bounds[0], bounds[1], Math.max(0, bounds[2] - bounds[0]),
                        Math.max(0, bounds[3] - bounds[1]));
                }
                this._render(program, this.framebuffers[0], [{name: 'u_image', texture: source}],
            ...
            skin._texture = output;
            if (skin._framebuffer.attachments) skin._framebuffer.attachments[0] = output;
            this.textures[0] = source;
            if (group) {
                group.texture = output;
                group.buffer.texture = output;
            }
            if (matte) {
                const buffer = matte.source.texture === source ? matte.source : matte.mask;
                buffer.texture = output;
            }
            if (this.frameTransaction && this.frameTransaction.stagingTexture === source) {
                this.frameTransaction.stagingTexture = output;
            }
            this._markSkinChanged(skin);
            if (group) group.bounds = bounds;
            return true;
        }
        ...
        _singlePass (program, uniforms, integerUniforms, blendMode) {
            ...
            if (this._directSinglePass(program, uniforms, integerUniforms, blendMode)) return;
